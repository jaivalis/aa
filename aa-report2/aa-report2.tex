...
\STATE Initialise s
\FORALL{Steps of episodes}
\STATE Choose Choose $a$ with probability $\frac{\exp^{Q_t(a)/r}}{\sum{\exp^{Q_t(b)/r}}}$ \label{soft}
\STATE Take action $a$, observe $r$, $s'$
\STATE $Q(s,a) \leftarrow Q(s,a) + \alpha \lbrack r + \gamma max_{a'} Q(s',a') - Q(s,a)\rbrack$
\STATE $s \leftarrow s';$\
\ENDFOR
...
\noindent Where:
\begin{itemize}
	\item $r$ is a positive parameter called \emph{temperature}. A high temperature leads the actions to be almost equiproble, low tempreature values cause a greater difference with $r$ $\rightarrow$ 0 beeing the same as greedy action selection.
\end{itemize}
\vspace*{10mm}
