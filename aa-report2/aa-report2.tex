...
\resizebox{200pt}{!}{
\begin{tikzpicture}
\begin{axis}[ymin=0,ymax=500,restrict y to domain=0:500,xlabel={Learned Episodes},ylabel={Average Performance}, legend style={at={(1.2,1)},anchor=north,legend cell align=left}]
...
\caption{Performance of agents learning with softmax action selection policy over time with different $\tau$ compared to an agent learning with 0.1-$\epsilon$-greedy action selection policy.}
\label{Exp3}
\end{figure}
\end{center}
%--3) what does it show (and is that what you expected, and why)
