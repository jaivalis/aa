...
Off-Policy Monte Carlo uses generated episodes from different policies. Then a tail of state/action pair is selected from each episode in order to get the part of the learned policy which has differences with the exploratory policy.
a weight is produced from the probabilities for the exploratory policy of selecting a certain action for a certain state.
This weight is subsequently used to calculate the Q(s,a), which will be used in the end for the generation of a deterministic greedy policy.
...
In this experiment we will compare and contrast a variety of other learning methods instead of Q-Learning, namely: On-Policy Monte-Carlo Control and Sarsa. By the end of this experiment we will have an overview of the different algorithms and we will be able to compare them
...
There is no unique way to pick the best performing combination of parameters, however we assumed that the best performing model is the one that outperformed the rest for the optimal combination of parameters. In that sense, finding an optimal candidate from the Softmax was non trivial. According to the previous statement the following models were picked:
\begin{itemize}
	\item{Q-Learning:} $Q_0$ = 15, $\epsilon$ = 0.1, $\alpha$ = 0.1, $\gamma$ = 0.9
	\item{Softmax:} $Q_0$ = 15, $\tau$ = 0.6, $\alpha$ = 0.5, $\gamma$ = 0.9
	\item{On-Policy Monte Carlo:} $Q_0$ = 15, $\gamma$ = 0.7
	\item{Sarsa:} $Q_0$ = 15, $\gamma$ = 0.9, $\alpha$ = 0.5, $\epsilon$ = 0.1
\end{itemize}
...
Although these models perform almost equally well, as seen previously, the best Softmax model outperforms the best other models in general. 

\section{Conclusion}
For this experiment alternate algorithms, that are based on model-free learning, were implemented. The first approach was with temporal difference methods with Q-Learning with different action selection schemes, namely $\epsilon$-greedy and Softmax. The second approach involved different learning methods like Monte Carlo Control and Sarsa. Finally a comparison and explanation between the various implemented approaches is provided.
